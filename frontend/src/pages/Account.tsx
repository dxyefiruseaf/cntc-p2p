...
export default function Account() {
  ...
  const saveProfile = async () => {
    ...
    setSaving(true);
    try { await auth.updateProfile(name.trim()); await auth.refreshProfile(); showToast('Đã cập nhật hồ sơ.', 'success'); }
    catch (reason) { showToast(reason instanceof Error ? reason.message : 'Không cập nhật được hồ sơ.', 'error'); }
  ...
  const changePassword = async () => {
    ...
    setSaving(true);
    try { await auth.updatePassword(password); setPassword(''); setConfirmPassword(''); showToast('Đã cập nhật mật khẩu.', 'success'); }
    catch (reason) { showToast(reason instanceof Error ? reason.message : 'Không đổi được mật khẩu.', 'error'); }
    finally { setSaving(false); }
  };
  const logout = async () => { setSaving(true); await auth.signOut(); navigate('/login', { replace: true }); };
  ...
  return <div className="page-enter space-y-5">
    <header><Badge variant="info">Account Center</Badge><h1 className="mt-2 text-2xl font-extrabold">Tài khoản của bạn</h1><p className="mt-1 text-sm text-[var(--text-sec)]">Quản lý hồ sơ, bảo mật, gói Sandbox và hoạt động gần đây.</p></header>
    {loading ? <div className="grid gap-4 lg:grid-cols-[.7fr_1.3fr]"><Skeleton className="h-96"/><Skeleton className="h-96"/></div> : <section className="grid gap-4 lg:grid-cols-[.72fr_1.28fr]">
      ...
      <div className="space-y-4"><Card className="p-5"><SectionHeader title="Thông tin cá nhân" sub="Tên hiển thị được lưu trong Supabase Auth."/><div className="grid gap-4 sm:grid-cols-2"><Input label="Họ và tên" value={name} onChange={event => setName(event.target.value)}/><Input label="Email" value={String(auth.user?.email || '')} disabled/></div><Button loading={saving} onClick={saveProfile} className="mt-4">Lưu thay đổi</Button></Card>
      <Card className="p-5"><SectionHeader title="Đổi mật khẩu" sub="Mật khẩu mới cần tối thiểu 8 ký tự."/><div className="grid gap-4 sm:grid-cols-2"><Input label="Mật khẩu mới" type="password" autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)}/><Input label="Xác nhận mật khẩu" type="password" autoComplete="new-password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)}/></div><Button variant="secondary" loading={saving} onClick={changePassword} className="mt-4">Cập nhật mật khẩu</Button></Card>
      ...
      <Card className="border-[#EF4444]/25 p-5"><SectionHeader title="Phiên đăng nhập" sub="Đăng xuất khỏi thiết bị này và xóa cache phiên cục bộ."/><Button variant="danger" loading={saving} onClick={() => void logout()}>Đăng xuất</Button></Card></div>
    </section>}
  </div>;
}
