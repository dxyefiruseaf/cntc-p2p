...
export default function Account() {
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      showToast(message.toLowerCase().includes('reauthentication')
        ? 'Phiên đăng nhập cần xác thực lại. Hãy đăng xuất, dùng Quên mật khẩu để xác minh OTP rồi thử lại.'
        : message, 'error');
    } finally {
  ...
  return <div className="page-enter space-y-5">
    ...
    {loading ? <div className="grid gap-4 lg:grid-cols-[.7fr_1.3fr]"><Skeleton className="h-96"/><Skeleton className="h-96"/></div> : <section className="grid gap-4 lg:grid-cols-[.72fr_1.28fr]">
      ...
      <Card className="p-5"><SectionHeader title="Tạo hoặc đổi mật khẩu" sub="Mật khẩu dùng để đăng nhập hằng ngày. OTP được dùng khi đăng ký mới hoặc khôi phục mật khẩu."/><div className="grid gap-4 sm:grid-cols-2"><Input label="Mật khẩu mới" type="password" autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)} placeholder="Tối thiểu 8 ký tự"/><Input label="Xác nhận mật khẩu" type="password" autoComplete="new-password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} placeholder="Nhập lại mật khẩu" onKeyDown={event => { if (event.key === 'Enter') void changePassword(); }}/></div><p className="mt-3 text-xs text-[var(--text-sec)]">Mật khẩu cần tối thiểu 8 ký tự và nên có cả chữ lẫn số. Sau khi cập nhật, hãy dùng email và mật khẩu này ở màn hình đăng nhập.</p><Button variant="secondary" loading={passwordSaving} onClick={changePassword} className="mt-4">Cập nhật mật khẩu</Button></Card>
