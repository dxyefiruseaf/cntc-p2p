...
def calculate_risk_score(latest: dict[str, Any], data_status: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def add(name: str, impact: float, note: str, value: Any = None) -> None:
        factors.append({"name": name, "impact": round(impact, 2), "value": value, "note": note})
    ...
    if rsi is None:
        ...
        add("RSI", 14, "RSI rất thấp, giá có thể hồi nhưng biến động ngắn hạn cao.", rsi)
    elif rsi <= 30:
        add("RSI", 9, "RSI dưới 30, thị trường quá bán nhưng chưa chắc đảo chiều ngay.", rsi)
    else:
    ...
    if macd_hist is None:
        ...
        add("MACD", 2, "MACD histogram dương, động lượng chưa phải rủi ro chính.", macd_hist)
    ...
    if close and ema_200:
        if close < ema_200:
            add("EMA200", 14, "Giá nằm dưới EMA200, xu hướng dài hạn cần thận trọng.", ema_200)
        else:
    ...
    if bb_width is None:
        ...
        if bw_pct >= 5:
            add("Bollinger width", 18, "Bollinger Bands mở rộng mạnh, biến động đang cao.", round(bw_pct, 3))
        elif bw_pct >= 3:
            add("Bollinger width", 12, "Bollinger Bands mở rộng, thị trường biến động đáng chú ý.", round(bw_pct, 3))
        elif bw_pct >= 1.8:
            add("Bollinger width", 6, "Biên độ Bollinger ở mức vừa phải.", round(bw_pct, 3))
        else:
    ...
    if close and atr:
        ...
        if atr_pct >= 2:
            add("ATR", 20, "ATR cao so với giá, rủi ro biến động lớn.", round(atr_pct, 3))
        elif atr_pct >= 1:
    ...
    if volume and vol_ma_20:
        ...
        if ratio >= 1.8:
            add("Volume", 9, "Khối lượng cao bất thường so với trung bình 20 kỳ.", round(ratio, 3))
        elif ratio <= 0.55:
            add("Volume", 5, "Thanh khoản thấp hơn trung bình, tín hiệu có thể kém tin cậy.", round(ratio, 3))
        else:
    ...
    if data_status:
        ...
        if ohlcv_age is None:
            ...
            add("Data freshness", 10, "Dữ liệu giá hơi cũ, nên kiểm tra pipeline.", ohlcv_age)
        else:
    ...
    label_vi = {"LOW": "Rủi ro thấp", "MEDIUM": "Rủi ro trung bình", "HIGH": "Rủi ro cao"}[level]
    recommendation = {
...
def generate_market_alerts(
    ...
    def push(severity: str, title: str, message: str, metric: str, value: Any = None) -> None:
        alerts.append({"severity": severity, "title": title, "message": message, "metric": metric, "value": value})
    ...
    if data_status:
        if data_status.get("is_ohlcv_fresh") is False:
            push("danger", "Dữ liệu giá có thể đã cũ", "Không nên ra quyết định khi pipeline chưa cập nhật dữ liệu giá mới.", "ohlcv_age_hours", data_status.get("ohlcv_age_hours"))
        if data_status.get("is_p2p_fresh") is False:
            push("warn", "Dữ liệu P2P chưa tươi", "Kết quả so sánh P2P/thực nhận nên được xem là tham khảo.", "p2p_age_hours", data_status.get("p2p_age_hours"))

    if rsi is not None:
        if rsi >= 75:
            push("danger", "RSI quá mua mạnh", "RSI trên 75, rủi ro điều chỉnh ngắn hạn tăng.", "rsi_14", rsi)
        elif rsi >= 70:
            push("warn", "RSI quá mua", "RSI trên 70, nên thận trọng với lệnh mua mới.", "rsi_14", rsi)
        elif rsi <= 25:
            push("warn", "RSI quá bán mạnh", "Giá có thể hồi nhưng biến động vẫn cao, không nên bắt đáy vội.", "rsi_14", rsi)

    if macd_hist is not None and macd_hist < 0:
        push("warn", "Động lượng MACD yếu", "MACD histogram âm, lực tăng chưa rõ ràng.", "macd_hist", macd_hist)

    if close is not None and bb_upper is not None and close >= bb_upper:
        push("warn", "Giá gần dải trên Bollinger", "Giá đang ở vùng dễ bị chốt lời ngắn hạn.", "bb_upper", bb_upper)
    if close is not None and bb_lower is not None and close <= bb_lower:
        push("warn", "Giá gần dải dưới Bollinger", "Giá ở vùng quá bán, cần xác nhận thêm trước khi mua.", "bb_lower", bb_lower)

    if close and atr:
        atr_pct = atr / close * 100
        if atr_pct >= 2:
            push("danger", "ATR cao", "Biến động hiện tại lớn so với giá, nên giảm quy mô giao dịch demo.", "atr_pct", round(atr_pct, 3))
        elif atr_pct >= 1:
            push("warn", "ATR tăng", "Biến động đáng chú ý, cần đặt ngưỡng rủi ro rõ ràng.", "atr_pct", round(atr_pct, 3))
    ...
    for row in p2p_rows or []:
        ...
        if spread is None or trade_type not in {"BUY", "SELL"}:
            continue
        if abs(spread) >= 1.5:
            push("warn", f"P2P {trade_type} lệch mạnh", f"Spread P2P {trade_type} đang lệch {spread:.3f}% so với giá tham chiếu.", f"p2p_spread_{trade_type.lower()}", spread)
    if not alerts:
        push("info", "Chưa có cảnh báo mạnh", "Các rule hiện tại chưa phát hiện tín hiệu rủi ro nổi bật.", "market", None)
